  ...
  return (
    <div className="space-y-24">
      <section className="relative overflow-hidden pt-20 pb-32 flex flex-col items-center text-center">
        ...
        <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-cyan-300 opacity-20 blur-[100px] rounded-full mix-blend-multiply" />
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8 animate-fade-in-up">
          ...
          <p className="mt-4 text-xl md:text-2xl text-slate-600 max-w-2xl mx-auto font-medium">
            Explore college-scoped clubs, approved public events, and the real
            activity now synced into your campus network.
          </p>
          <div className="mt-10 flex flex-col sm:flex-row justify-center gap-4">
            <a
              href="#clubs"
              className="px-8 py-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl shadow-lg hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 font-semibold text-lg"
            >
              Explore Clubs
            </a>
            <Link
              to="/create-club"
              className="px-8 py-4 bg-white text-indigo-600 border border-indigo-100 rounded-xl shadow-sm hover:shadow-xl hover:bg-slate-50 hover:-translate-y-1 transition-all duration-300 font-semibold text-lg"
            >
              Start a Club
            </Link>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-32 pb-32">
        <section id="clubs" className="scroll-mt-24 space-y-10">
          <div className="flex justify-between items-end border-b border-slate-200 pb-4">
            <div>
              ...
              <p className="text-slate-500 mt-2">
                Join dynamic groups fueled by passion.
              </p>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <p className="text-slate-500 py-12 text-center text-lg bg-slate-50 rounded-2xl border border-dashed border-slate-300">
              No active clubs found for the selected college yet.
            </p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {clubs.map((club) => (
                <div
                  key={club.id}
                  className="group relative bg-white rounded-2xl border border-slate-200 p-8 hover:border-indigo-200 hover:shadow-2xl hover:shadow-indigo-500/10 transition-all duration-300 hover:-translate-y-2 overflow-hidden"
                >
        ...
        <section id="events" className="scroll-mt-24 space-y-10">
          <div className="flex justify-between items-end border-b border-slate-200 pb-4">
            <div>
              ...
              <p className="text-slate-500 mt-2">
                Engage with what&apos;s happening around campus.
              </p>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <p className="text-slate-500 py-12 text-center text-lg bg-slate-50 rounded-2xl border border-dashed border-slate-300">
              No upcoming public events for the selected college yet.
            </p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {events.map((event) => (
                <Link
                  to={`/events/${event.id}`}
                  key={event.id}
                  className="group bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-xl hover:border-sky-200 transition-all duration-300 flex flex-col sm:flex-row"
                >
                  <div className="bg-gradient-to-br from-sky-500 to-indigo-600 sm:w-1/3 p-6 flex flex-col justify-center items-center text-white text-center">
                    <span className="text-4xl font-extrabold">
                      {new Date(event.date).getDate()}
                    </span>
                    <span className="text-lg font-medium opacity-90 uppercase tracking-wider">
